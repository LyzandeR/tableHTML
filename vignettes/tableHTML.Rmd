...
### Taking a picture of the tableHTML
...
However, we have included three categories of themes (scientific, rshiny-blue and totals) to give the users a 
...
And in order to align the first of the second_headers in the right our css would look like:
...
mtcars %>%
  ...
  add_css_column(list('border-left', '5px solid red'), columns = 1) %>%
  add_css_header(list('background-color', 'green'), 2) %>%
  add_css_second_header(list('text-align', 'right'), 1)
```
