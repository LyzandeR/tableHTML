...
The package offers three themes for now: scientific, rshiny-blue and totals. To use them, you need to use the
`add_theme` function. The first two are meant for any table whereas the totals theme is mainly designed
for tables showing a summary metric on the final row.
...
This is the scientifc theme where the table design corresponds to scientific tables for publishing.
...
This theme was mainly designed for highlighting the final row of a data.frame. The function `add_themes` 
can be used for totals in a quick way, but to read about the arguments that you can use with totals please 
visit `?add_theme_totals`.
...
df %>%
  tableHTML(widths = c(150, rep(70, ncol(df))), rownames = TRUE) %>%
  add_theme('totals', color = 'steelblue')
```
...
df %>%
  tableHTML(widths = c(150, rep(70, ncol(df))), rownames = TRUE) %>%
  add_theme('totals', color = c('steelblue', 'red'))
```

## Totals with add_theme_totals
