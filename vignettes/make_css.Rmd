...
The first one would be directly at ui.R using `tags$style` (which is what `includeCSS` uses behind
the scenes after it loads the file):
...
The second way is the obvious way i.e. creating a file with all the css and then loading it in shiny
using `includeCSS` in the way it was described in the 
[tableHTML tutorial](https://lyzander.github.io/tableHTML/#shiny-and-css). As a fast example we 
create a css file using `make_css`:


```{r shiny css 2}
#when workin on shiny the css file is best placed in the www/ folder of the shiny app.
