...
This example uses the famous Iris dataset. Instead of using the default headers, 
only `'Length'` and `'Width'` are used. The prefixes `'Sepal'` and `'Petal'` are 
shown in the second headers above. The `'Species'` is moved to the row_groups, 
spanning across all rows of each species. (Note: the `<div>` container is introduced,
...
With the colour rank conditional css on the data, it is already visible why the species
is a relatively easy target to predict.
...
tableHTML(iris[1:4], 
          ...
          row_groups = list(rep(70, 3), paste0('<div style="width:100%; height:100%; ',
