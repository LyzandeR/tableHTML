...
mtcars %>% 
 `[<-`('Average', 
       1:ncol(mtcars),  
       value=round(colMeans(mtcars), 2)) %>% 
 `[`(c(1, 3:7)) %>% 
tableHTML(rownames = TRUE,
          widths = c(140, rep(50, 6))) %>%
 add_theme('totals')
```

### customized totals theme

```{r example 7.1}
mtcars %>% 
 tableHTML(rownames = TRUE,
          widths = c(140, rep(50, 11))) %>%
 add_theme_totals(color='steelblue', total_rows=NULL)
```
...
c('AVG', round(mean(ts_to_df[13:24, ]$AirPassengers), 2))) %>% 
  ...
  add_theme_totals(id_column = TRUE, 
                   total_rows = c(13, 26), 
...
```{r example 21, fig.height=7, fig.width=7}
mtcars %>%
 tableHTML(widths = c(140, rep(50, 11))) %>%
 add_theme('rshiny-blue') %>%
 tableHTML_to_image(type = 'png')
